...
// Hand-generated OpenAPI 3.1 spec for the v1 API. We don't pull in
// zod-to-openapi: the route schemas are small and inline, so generating the
// document directly keeps the dependency tree lean. Served at
// GET /api/v1/openapi.json and rendered by /settings/api/docs.
...
function op(args: {
  ...
  return {
    ...
    ...(args.scope
      ? { description: `Required scope: \`${args.scope}\``, security: [{ bearerAuth: [] }] }
  ...
  return {
    ...
    paths: {
      ...
      "/api/v1/time-entries/{id}/approve": {
        post: op({ tag: "time-entries", summary: "Approve a time entry", scope: "time-entries:approve", params: [idParam], successStatus: "200", success: dataEnvelope(genericObject) }),
      },
...
export async function GET() {
  return NextResponse.json(buildSpec(), { headers: { "Cache-Control": "no-store" } });
}
